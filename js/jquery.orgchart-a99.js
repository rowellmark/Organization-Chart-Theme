...
(function(factory) {
  ...
  function buildJsonDS($li) {
    var subObj = {
      ...
      'contents': $li.find('> em').text(),
      'type' : $li.data('type'),
      'custom_class' : $li.data('class'),
      'link': $li.data('link')
    };
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div ' + (nodeData['type'] != undefined ? 'data-type="' + nodeData['type'] + '"' : '') + (nodeData['link'] != undefined ? 'data-link="' + nodeData['link'] + '"' : '') + (nodeData['custom_class'] != undefined ? 'data-class="' + nodeData['custom_class'] + '"' : '')  +  (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
      .addClass('node ' + (nodeData.className || '') +  (level >= opts.depth ? ' slide-up' : ''))
      .append('<div class="node-center"></div>');

      $nodeDiv.find('.node-center')
      .append('<div class="title">' + nodeData[opts.nodeTitle] + '</div>')
      .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (nodeData[opts.nodeContent] || '') + nodeData['contents'] + '</div>' : '');
